...
# prediction
RF.prediction <- predict(RF_modfit, cross.val)

confusionMatrix(RF.prediction, cross.val$classe)
```

Plot the accuracy level
```{r}
plot(RF.prediction$table, col = RF.prediction$byClass, 
     main = paste("RF - Accuracy Level =",
                  round(RF.prediction$overall['Accuracy'], 4)))
```
